...
def test_auto_play_is_enqueued_after_derivatives_finish(tmp_path: Path, monkeypatch) -> None:
    ...
    async def scenario():
        for action in bot.PLAY_SEQUENCE:
            await bot._enqueue_derivative_job("derive", note_id, action, message)
        await bot._wait_for_background_jobs()
...
def test_handle_note_action_enqueues_explanation_and_then_serves_markdown(tmp_path: Path, monkeypatch) -> None:
    ...
    assert len(followup_message.document_replies) == 1
...
def test_handle_note_action_play_resumes_only_missing_actions_after_restart(tmp_path: Path, monkeypatch) -> None:
    ...
    assert bot._play_audio_path(note_id, "suggest").exists()
    assert bot._play_audio_path(note_id, "research").exists()
    assert bot._play_audio_path(note_id, "dialectic").exists()
    assert bot._play_state_path(note_id).exists()
