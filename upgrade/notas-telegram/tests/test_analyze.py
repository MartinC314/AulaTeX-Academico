...
from src.analyze import _build_analysis_request, _build_json_repair_request, _detect_text_type, _extract_json, analyze_text
from src.azure_openai_client import build_pdf_input_message, build_responses_payload, extract_response_text, invoke_chat, uses_openai_v1_endpoint
...
def test_analyze_text_supports_openai_v1_endpoint(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("src.azure_openai_client.OpenAI", DummyOpenAI)
...
def test_analyze_text_requires_azure_openai(tmp_path) -> None:
    settings = make_settings(tmp_path)
...
def test_analyze_text_retries_when_first_response_is_invalid_json(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("src.analyze.invoke_chat", fake_invoke_chat)
...
def test_invoke_chat_retries_v1_incomplete_response_without_text(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("src.azure_openai_client.OpenAI", IncompleteThenCompleteOpenAI)
