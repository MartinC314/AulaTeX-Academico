...
from src.notes import build_derivative_display_title, build_derivative_markdown, build_display_title, build_note_markdown, derivative_filename, save_note, save_note_derivative, sentence_case, slugify_title
...
def test_sentence_case_formats_title_like_sentence() -> None:
    ...
    assert sentence_case("¿QUE PASA CON AZURE?") == "¿Que pasa con azure?"
...
def test_build_derivative_display_title_uses_note_title_and_action() -> None:
    assert build_derivative_display_title("20:17 - Contexto cultural", "dialectic") == (
        "20:17 - Contexto cultural · Dialectica"
    )
...
def test_save_note_derivative_writes_markdown_and_updates_note_links(tmp_path) -> None:
    ...
    assert f"# {saved.title} · Explicar" in derivative_markdown
...
def test_build_derivative_markdown_preserves_structured_sections_from_model_output(tmp_path) -> None:
    saved = save_note(
        tmp_path,
        {
            "title": "Conocer más allá de percibir",
            "corrected_text": "Texto base.",
            "concepts": [],
            "related_terms": [],
        },
