...
\begin{landscape}
\thispagestyle{empty}
\newgeometry{top=0.7cm,bottom=0.65cm,left=0.75cm,right=0.75cm}
\begin{center}
...
\begin{tikzpicture}[
  ...
  fundamento/.style={rectangle,rounded corners=3pt,draw=unadmGreenDark,thick,fill=unadmBlue,align=left,text width=7.15cm,minimum height=3.65cm,inner sep=8pt,font=\small},
  ...
  enlace/.style={-{Stealth[length=2.3mm]},thick,draw=unadmGreenDark},
  oro/.style={-{Stealth[length=2.3mm]},thick,draw=unadmGold}
]

\node[principal] (p) at (0,0) {PENSIONISSSTE\\[-1pt]\small Fondo Nacional de Pensiones de los Trabajadores al Servicio del Estado};

\node[fundamento] (f1) at (-8.0,-3.25) {\textbf{1. Naturaleza y utilidad}\\[4pt]
...
\node[fundamento] (f2) at (0,-3.25) {\textbf{2. Pensión: naturaleza y finalidad}\\[4pt]
...
\node[fundamento] (f3) at (8.0,-3.25) {\textbf{3. Cuenta individual}\\[4pt]
...
\draw[enlace] (p.south west) -- (f1.north);
\draw[enlace] (p.south) -- (f2.north);
\draw[enlace] (p.south east) -- (f3.north);

\node[banda] (flujoT) at (0,-6.15) {Funcionamiento: del ahorro al pago de la pensión};
\node[proceso] (a) at (-9.4,-7.75) {1. Cuotas y aportaciones\\Trabajador, dependencia y Estado};
\node[proceso] (b) at (-4.7,-7.75) {2. Individualización\\Registro en subcuentas};
\node[proceso] (c) at (0,-7.75) {3. Inversión\\Rendimientos regulados};
\node[proceso] (d) at (4.7,-7.75) {4. Reconocimiento\\ISSSTE verifica requisitos};
\node[proceso] (e) at (9.4,-7.75) {5. Elección y pago\\Modalidad pensionaria};
\draw[enlace] (a)--(b); \draw[enlace] (b)--(c); \draw[enlace] (c)--(d); \draw[enlace] (d)--(e);

\node[banda,fill=unadmGold,draw=unadmGold,text=unadmInk] (pensT) at (0,-9.35) {Pensiones y requisitos en el régimen de cuentas individuales};
\node[pension] (r1) at (-9.0,-12.15) {\textbf{A. Retiro anticipado}\\\textit{Art. 80}\\[4pt]
...
\node[pension] (r2) at (-3.0,-12.15) {\textbf{B. Cesantía en edad avanzada}\\\textit{Arts. 84--87}\\[4pt]
...
\node[pension] (r3) at (3.0,-12.15) {\textbf{C. Vejez}\\\textit{Arts. 88--91}\\[4pt]
...
\textbf{Otorga:} pensión y seguro de salud.};
\node[pension] (r4) at (9.0,-12.15) {\textbf{D. Pensión garantizada}\\\textit{Arts. 92--96}\\[4pt]
...
\draw[oro] (pensT.south) -- ++(0,-0.35) -| (r1.north);
\draw[oro] (pensT.south) -- ++(0,-0.35) -| (r2.north);
\draw[oro] (pensT.south) -- ++(0,-0.35) -| (r3.north);
\draw[oro] (pensT.south) -- ++(0,-0.35) -| (r4.north);

\node[banda] (modT) at (0,-15.25) {Modalidades de pago para cesantía y vejez};
\node[modalidad] (m1) at (-6.0,-17.0) {\textbf{Renta vitalicia.} Seguro de pensión contratado con la aseguradora elegida. Paga durante la vida de la persona pensionada, se actualiza en febrero conforme al INPC y exige cubrir el seguro de sobrevivencia. La elección no permite regresar a retiro programado.};
\node[modalidad] (m2) at (6.0,-17.0) {\textbf{Retiros programados.} El saldo permanece en PENSIONISSSTE o en una administradora y financia pagos periódicos recalculados. Puede cambiarse a renta vitalicia si la renta convenida no resulta inferior a la pensión garantizada.};
\draw[enlace] (modT.south) -- ++(0,-0.25) -| (m1.north);
\draw[enlace] (modT.south) -- ++(0,-0.25) -| (m2.north);
\end{tikzpicture}%
}
\vspace{0.22cm}

{\footnotesize\textit{Figura 1. Elaboración propia con base en los artículos 76--113 de la LISSSTE y los materiales de la semana 8 \citep{lissste2026,materialS8DSS2026}.}}
\end{center}
\restoregeometry
\end{landscape}
\clearpage
