...
\begin{tikzpicture}[
 ...
 concept/.style={rounded corners=2pt,draw=itescaCharcoal!65,fill=white,font=\footnotesize,align=center,text width=3.8cm,minimum height=1.35cm},
...
\node[branch] (bd) at (-7.8,11.8) {Bolman y Deal\\cuatro marcos};
\node[branch] (mo) at (0,11.8) {Morgan\\ocho metáforas};
\node[branch] (gl) at (7.8,11.8) {Perspectiva global\\lectura transversal};
...
\node[concept] (es) at (-10,7.7) {Estructural\\roles, reglas, metas\\y coordinación};
\node[concept] (rh) at (-5.6,7.7) {Recursos humanos\\necesidades, capacidades\\y participación};
\node[concept] (po) at (-10,3.8) {Político\\intereses, conflicto,\\poder y negociación};
\node[concept] (si) at (-5.6,3.8) {Simbólico\\cultura, rituales,\\relatos y sentido};
\foreach \n in {es,rh,po,si} \draw[edge] (bd) -- node[label]{incluye} (\n);

\node[concept] (m1) at (-2.2,7.7) {Máquina / organismo\\orden y adaptación};
\node[concept] (m2) at (2.2,7.7) {Cerebro / cultura\\aprendizaje y significado};
\node[concept] (m3) at (-2.2,3.8) {Sistema político /\\prisión psíquica};
\node[concept] (m4) at (2.2,3.8) {Flujo y transformación /\\instrumento de dominación};
\foreach \n in {m1,m2,m3,m4} \draw[edge] (mo) -- node[label]{representa como} (\n);

\node[concept] (g1) at (5.6,7.7) {Economía y tecnología\\transforman capacidades};
\node[concept] (g2) at (10,7.7) {Política y cultura\\modifican reglas y sentido};
\node[concept] (g3) at (5.6,3.8) {Interdependencia\\vincula actores y países};
\node[concept] (g4) at (10,3.8) {Adaptación responsable\\responde al entorno};
